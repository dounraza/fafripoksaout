...
const Player = ({
    ...
    useEffect(() => {
        ...
        if (isWinner || isLoser) {
            ...
            if (!isRevealFinished) {
                ...
                setIsStackHidden(true); // Cacher le stack dès qu'on sait qu'il y a un showdown
            } else {
                ...
                if (!showResult) {
                    ...
                    const timerStack = setTimeout(() => {
                        ...
                        lastSafeStack.current = actualStack;
                        isLocked.current = false;
                    }, 5000); // 5 secondes de suspense total
    ...
    useEffect(() => {
        if (shouldShareCards && playerRefs[i]?.current && tableRef.current) {
            ...
            let currentZoom = 1;
            if (window.innerWidth <= 451 && window.innerWidth > 399) currentZoom = 0.9;
            if (window.innerWidth <= 399 && window.innerWidth > 350) currentZoom = 0.8;
            if (window.innerWidth <= 350) currentZoom = 0.7;

            // Animation de départ : Centre de la table
            ...
            >
                ...
                <div className={`player-cards ${cardCount > 2 ? 'omaha' : ''}`}>
                    {(winData?.allCards ?? []).length > 0 ? (
                        <div className={`card-containers ${cardCount > 2 ? 'omaha' : ''}`} style={{ marginTop: '1.2em' }}>
                            ...
                            {shouldShareCards && (
                                ...
                                >
                                    {[...Array(cardCount)].map((_, idx, arr) => {
                                        ...
                                        const finalX = sharingCards ? 0 : dx + (xOffset / 100) * 30; // Scale xOffset for better centering

                                        // Sequential delay: one card after another for each player
 ...
 <div className="player-action-status">
                ...
                {/* Barre de progression du temps (Countdown) */}
                {tableState.toAct === i && (
